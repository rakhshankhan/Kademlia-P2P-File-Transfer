...
function communicate(clientSocket, clientName, clientDHTtable) {
  ...
  clientSocket.on("data", (message) => {
    ...
    if (kadPacket.msgType == 1) {
      ...
      console.log(
        "The message type " + kadPacket.msgType + " is not supported"
      );
    }
  });
...
function getMessage(msg) {
  let imgSocket = new net.Socket();

  let senderNameSize = parseBitPacket(msg, 20, 12);

  let firstIP = parseBitPacket(msg, 32 + senderNameSize * 8, 8);
  let secondIP = parseBitPacket(msg, 40 + senderNameSize * 8, 8);
  let thirdIP = parseBitPacket(msg, 48 + senderNameSize * 8, 8);
  let forthIP = parseBitPacket(msg, 56 + senderNameSize * 8, 8);

  let ip = firstIP + "." + secondIP + "." + thirdIP + "." + forthIP;
  let port = parseBitPacket(msg, 64 + senderNameSize * 8, 16);

  let imgType = parseBitPacket(msg, 96 + senderNameSize * 8, 4);
  ...
  let imgNameSize = parseBitPacket(msg, 100 + senderNameSize * 8, 28);
  let imgName = bytes2string(
    msg.slice(16 + senderNameSize, 16 + senderNameSize + imgNameSize)
  );

  try {
    let imageFullName = "images/" + imgName + "." + imgTypeName;
    ...
    imgSocket.connect(port, ip, () => {
      imgSocket.write(ITPpacket.getBytePacket());
      imgSocket.end();
    });
  } catch (err) {}
}
